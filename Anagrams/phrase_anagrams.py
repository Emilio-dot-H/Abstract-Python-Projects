...
# 1 - SETTING UP AND FINDING ANAGRAMS
...
ini_name = input("Enter a word/name: ")

def find_anagrams(name, word_list):
	"""Read name & dictionary file & display all anagrams IN name"""
	#Number of times a letter appears in a name
	name_letter_map = Counter(name)
	anagrams = []

	for word in word_list:
		# test: accumulates all the letters in the word that 'fit' in the name
		test = ''
		word_letter_map = Counter(word.lower())
		for letter in word:
			#check that the count of each letter is the same as/less than, the count in name.
...
# 2 - PROCESSING THE USER'S CHOICE
def process_choice(name):
	"""Check user choice for validity, return choice & leftover
	letters"""
	while True:
